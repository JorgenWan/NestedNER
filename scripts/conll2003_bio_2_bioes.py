...
def bio_to_bioes(in_file, out_file):
    insts = Instances.load_data(in_file, digit2zero=False)
...
if __name__ == "__main__":
    ...
    data_dir = "/NAS2020/Workspaces/NLPGroup/juncw/summer_dataset/reduce_emb_dataset/old_ner_2020_09_09"
    data_name = "conll2003"
    from_data_dir = f"{data_dir}/{data_name}"
    to_data_dir = f"{data_dir}/{data_name}/bioes"
    os.makedirs(to_data_dir, exist_ok=True)
    bio_to_bioes(f"{from_data_dir}/train.txt", f"{to_data_dir}/train.txt")
    bio_to_bioes(f"{from_data_dir}/dev.txt", f"{to_data_dir}/valid.txt")
    bio_to_bioes(f"{from_data_dir}/test.txt", f"{to_data_dir}/test.txt")